;(function($) {
	...
	SideMenu.prototype = {
		init: function () {
			...
			$('body').addClass('sidemenu-active').on( 'scrollhighlightstart.lazycrab', function() {
				if ( self.panelVisible ) {
					self.hidePanel();
				}
			}).on( 'sidemenushow.lazycrab', function(e, emitterPanel, side) {
		...
		showPanel: function () {
			...
			$(thisPanel).removeClass('sidemenu-off').addClass('sidemenu-on').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function(){
				$(this).trigger( 'sidemenuaftershow.lazycrab', [thisPanel] );
			});
			$('body').addClass(this.panelCleanName + '-visible sidemenu-' + this.settings.side).trigger('sidemenushow.lazycrab', [thisPanel, this.settings.side]);
		...
		hidePanel: function ( side ) {
			...
			$('body').removeClass(cssClass).trigger('sidemenuhide.lazycrab', [thisPanel]);
			this.panelVisible = false;
		}
	};

	$.fn.lazycrabSideMenu = function ( options ) {
